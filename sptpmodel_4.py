...
class SPTPmodel_4:
    ...
    def __init__(self, sptpData, isInteger=True, debug=False, options=None, model_options=None):
        ...
        :param sptpData: structure containing all data
             ...
             .getD(i,j) = d_ij maximum cost of requirement j that can be satisfied with asset i (matrix, [ij])
        ...
        self.name = sptpData.name + '_M_' + str(self.M) + '_N_' + str(self.N) + "_v4"
        ...
        def initD(model, i, j):
            return sptpData.getD(i,j)
        self.model.D = Param(self.model.I, self.model.J, initialize=initD, within=NonNegativeReals)
        ...
        def XijBounds_rule(model, i, j):
            uBound = sptpData.getA(i)/sptpData.getS(i,j)
            if sptpData.getP(i,j) > 0:
                return (0, min(uBound, sptpData.getD(i,j)/sptpData.getP(i,j)))
            else:
                return (0, uBound)
